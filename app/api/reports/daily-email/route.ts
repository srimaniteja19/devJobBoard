...
export async function POST(req: NextRequest) {
  ...
  const providedBearer = authHeader?.startsWith("Bearer ") ? authHeader.slice("Bearer ".length) : null;
  const providedX = req.headers.get("x-cron-secret");

  const secret = vercelCronSecret ?? dailyReportSecret;
  if (secret) {
    const provided = vercelCronSecret ? providedBearer : providedX ?? providedBearer;
